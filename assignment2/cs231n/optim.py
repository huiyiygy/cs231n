...
-------
  ...
  - config: A dictionary containing hyperparameter values such as learning rate, momentum, etc.
        If the update rule requires caching values over mangy iterations, then config will also
        hold these cached values.
...
NOTE: For most update rules, the default learning rate will probably not perform well;however
the default values of the other hyperparameters should work well for a variety of different
problems.

For efficiency, update rules may perform in-place updates, mutating w and setting next_w equal
to w.
"""
...
def sgd_momentum(w, dw, config=None):
    """
    Performs stochastic gradient descent with momentum. 执行动量方法的随机梯度下降
