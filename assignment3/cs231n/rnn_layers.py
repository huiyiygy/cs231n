...
def rnn_step_forward(x, prev_h, Wx, Wh, b):
    ...
    h(t) = f(h(t-1), x) = tanh(Wh * h(t-1) + Wx * x +b)
...
def rnn_step_backward(dnext_h, cache):
    ...
    N, H = prev_h.shape
    # compute the dtanh, dtanh = 1 - tanh(x)^2
    temp = np.ones((N, H)) - np.square(np.tanh(out))
    dtanh = np.multiply(dnext_h, temp)
    # compute the db
    tempb = np.sum(dtanh, axis=0)
    db = tempb.T
    # compute the dx
...
def rnn_forward(x, h0, Wx, Wh, b):
    ...
    h = np.empty([N, T, H])
    out = np.empty([N, T, H])
    prev_prev_h = np.empty([N, T, H])
    for i in range(0, T):
        temp_h, cache_temp = rnn_step_forward(x[:, i, :], prev_h, Wx, Wh, b)
        prev_prev_h[:, i, :] = prev_h
        h[:, i, :] = temp_h
        out[:, i, :] = cache_temp[4]
        prev_h = temp_h
    cache = (x, prev_prev_h, Wx, Wh, out)
    ##############################################################################
    #                               END OF YOUR CODE                             #
    ##############################################################################
    return h, cache
...
def rnn_backward(dh, cache):
    ...
    ##############################################################################
    x, prev_prev_h, Wx, Wh, out = cache
    N, T, D = x.shape
    _, _, H = prev_prev_h.shape
    ...
    for j in range(0, T):
        i = T - 1 - j
        dnow = dnow + dh[:, i, :]
        cache_i = (x[:, i, :], prev_prev_h[:, i, :], Wx, Wh, out[:, i, :])
        dx_temp, dprev_h, dWx_temp, dWh_temp, db_temp = rnn_step_backward(dnow, cache_i)
        dx[:, i, :] = dx_temp
        dnow = dprev_h
        dWx = dWx + dWx_temp
        dWh = dWh + dWh_temp
...
def lstm_forward(x, h0, Wx, Wh, b):
    ...
    for i in range(0, T):
        next_h, next_c, step_cache = lstm_step_forward(x[:, i, :], prev_h, prev_c, Wx, Wh, b)
...
def lstm_backward(dh, cache):
    ...
    #############################################################################
    D = cache[0][0].shape[1]  # step_x is (N, D)
    N, T, H = dh.shape
    ...
    for j in range(0, T):
        i = T - 1 - j
        step_dx, step_dprev_h, step_dprev_c, step_dWx, step_dWh, step_db = lstm_step_backward(dh[:, i, :] + step_dprev_h, step_dprev_c, cache[i])
        dx[:, i, :] = step_dx
        dWx = dWx + step_dWx
        dWh = dWh + step_dWh
        db = db + step_db
