...
@function:
@author:HuiYi or 会意
@file:StyleTransfer-TensorFlow.py
...
def gram_matrix(features, normalize=True):
    ...
    features = tf.reshape(features, (shape[0], shape[3], -1))  # (1, C, H*W)
    features_transpose = tf.transpose(features, [0, 2, 1])  # (1, H*W, C)
    covariance_matrix = tf.matmul(features, features_transpose)
    if normalize:
        covariance_matrix = covariance_matrix / tf.cast(shape[1] * shape[2] * shape[3], dtype=tf.float32)
...
def style_loss(feats, style_layers, style_targets, style_weights):
    ...
    for i in range(len(style_layers)):
        idx = style_layers[i]
        loss += style_weights[i] * tf.reduce_sum((gram_matrix(feats[idx]) - style_targets[i]) ** 2)
    return loss
...
def tv_loss(img, tv_weight):
    ...
    h_variation = tf.reduce_sum((img[:, 1:, :, :] - img[:, :-1, :, :]) ** 2)
    w_variation = tf.reduce_sum((img[:, :, 1:, :] - img[:, :, :-1, :]) ** 2)
